...
double SurveyMissionItem::_calculateSegmentFlightTime(const double segmentDistance) const
{
    ...
    ParameterManager *parameterLoader = _vehicle->parameterManager();
    double topSpeed    = parameterLoader->getParameter(-1, "WPNAV_SPEED")->rawValue().toDouble()/100; // m/s
    double accelration = parameterLoader->getParameter(-1, "WPNAV_ACCEL")->rawValue().toDouble()/100; // m/s/s
